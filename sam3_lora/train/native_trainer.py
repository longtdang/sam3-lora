...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        self.data_conf = data
        self.train_loader_instance = train_loader
        self.val_loader_instance = val_loader
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        if self.lora_conf:
            ...
            lora_state_dict = get_lora_state_dict(unwrap_ddp_if_wrapped(self.model))
            checkpoint = {
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if self.lora_conf and "lora_state_dict" in checkpoint:
            ...
            load_lora_state_dict(unwrap_ddp_if_wrapped(self.model), checkpoint["lora_state_dict"])
        else:
            load_state_dict_into_model(
                model=self.model,
    ...
    def _setup_dataloaders(self):
        ...
        if self.val_loader_instance:
            pass # Handled in run_val
        elif self.mode in ["train", "val"] and self.data_conf:
            self.val_dataset = instantiate(self.data_conf.get(Phase.VAL, None))

        if self.train_loader_instance:
            pass # Handled in run_train
        elif self.mode in ["train", "train_only"] and self.data_conf:
            self.train_dataset = instantiate(self.data_conf.train)

    def run_train(self):
        while self.epoch < self.max_epochs:
            if self.train_loader_instance:
                dataloader = self.train_loader_instance
                if hasattr(dataloader.sampler, "set_epoch"):
                    dataloader.sampler.set_epoch(int(self.epoch))
            else:
    ...
    def run_val(self):
        if self.val_loader_instance:
            dataloader = self.val_loader_instance
            if hasattr(dataloader.sampler, "set_epoch"):
                dataloader.sampler.set_epoch(int(self.epoch))
        ...
        if not self.val_loader_instance:
            del dataloader
            gc.collect()
        self.logger.log_dict(outs, self.epoch)  # Logged only on rank 0
    ...
    def _setup_components(self):
        ...
        val_phase = Phase.VAL
        val_keys = None
        if self.data_conf and self.data_conf.get(val_phase, None) is not None:
        ...
        if self.lora_conf:
            ...
            lora_config = LoRAConfig(**self.lora_conf)
            self.model = inject_lora_into_model(self.model, lora_config, verbose=True)
            
            logging.info("LoRA injected. Trainable parameters:")
            print_model_summary(self.model)
    ...
    def _construct_optimizers(self):
        if self.optimizer_instance is not None:
            self.optim = self.optimizer_instance
            return
