...
dataset/
  v1/
    ...
    _annotations.coco.json   ← COCO format from CVAT
  v2/
    ...
```

The script auto-detects `_annotations.coco.json` inside the given `--dataset-dir`.  
...
- COCO JSON not found → exit with clear error message
